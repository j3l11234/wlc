...
class ParttimeModel extends Model {
	...
	public function submitParttime($user_id, $parttime_id, $date, $type, $student_id, $name, $nature, $work){
		if(empty($parttime_id)){ //新增记录
			$data = array(
				'user_id'		=>	$user_id,
				'date'			=>	$date,
				'check_status'	=>	1,
			...
			//获取记录
			$data = $this->where(array('parttime_id' => $parttime_id))->select()[0];
			if(!$data)
				return null;
			//检查记录所有者
	...
	public function userQuery($user_id, $start_date='', $end_date='', $check_status = 4, $page = 1,$per_page = 8){
		...
		->field('parttime_id, date, type, student_id, name, nature, work,
			check_status, check_datetime, check_comment');
	 ...
	 * 
	 * @param int $user_id		用户id
	 * @param int $parttime_id		请假申请条目id
	 * @return 删除失败null 否则返回数据条目
	 */
	public function deleteParttime($user_id,$parttime_id,$admin=false){
		//获取记录
		$data = $this->where(array('parttime_id' => $parttime_id))->select()[0];
		if(!$data)
			return null;
		...
		$this->where(array('parttime_id' => $parttime_id))->delete();
		//检测是否删除成功
		if($this->where(array('parttime_id' => $parttime_id))->select()[0] != null)
			return null;
	...
	public function adminQuery($department_id = 0, $user_id = 0, $start_date='', $end_date='', $check_status = null, $order='', $page = 1,$per_page = 0){
		...
		if($user_id != 0)
			$where['wlc_parttime.user_id'] = $user_id;
		...
		//var_dump($where);
		$this->join('RIGHT JOIN wlc_user ON wlc_user.user_id = wlc_parttime.user_id')
		->join('LEFT JOIN wlc_department ON wlc_department.department_id = wlc_user.department_id');
		$count = $this->where($where)->count(); 
		//var_dump($where);
		$this->join('RIGHT JOIN wlc_user ON wlc_user.user_id = wlc_parttime.user_id')
		->join('LEFT JOIN wlc_department ON wlc_department.department_id = wlc_user.department_id');
		$this->field('parttime_id, wlc_user.alias, department_name, date, type, student_id, name, nature, work,
			check_status,check_datetime,check_comment')
	 ...
	 */
	public function approbateParttime($checker_id,$parttime_id,$is_agree,$check_date,$check_comment){
		//获取记录
		$data = $this->where(array('parttime_id' => $parttime_id))->select()[0];
		...
		$data['check_datetime']	=	date('Y-m-d H:i:s',$check_date);
		$data['check_comment']	=	$check_comment;


		$this->save($data);

		$result = $this->where(array('parttime_id' => $parttime_id))->select()[0];
		return $result;
	}
