	...
	<div class="panel-body">
				...
				<div class="row">	
					...
					<div class="col-lg-4 col-sm-6 form-div">
						...
						<div class="input-group">
							<select name="status" class="selectpicker">
								...
								<option value="11">未归还</option>
								<option value="12">已归还</option>
							</select>
						</div>
					</div>
					<div class="col-lg-4 col-sm-6 form-div">
					...
					<div class="col-lg-4 col-sm-6 form-div">
						<div style="input-group; text-align: center;">
							<button style="width:45%" type="button" onclick="addBorrow()" class="btn btn-success">新申请</button>
							&nbsp;
		...
		<div class="table-responsive">
			<table class="table table-condensed table-hover">
				...
				<tbody> 		
				<?php if(is_array($borrowList)): foreach($borrowList as $key=>$record): ?>
					...
					<?php else: ?> 
						...
						<td>
							<?php switch($record['check_status']): case "1": ?><button type="button" class="btn btn-xs btn-info" disabled="disabled">等待审批</button>
									<button type="button" class="btn btn-xs " style="visibility:hidden;">未归还</button><?php break;?>
								<?php case "2": ?><button type="button" class="btn btn-xs btn-success" disabled="disabled">审批通过</button>
									<?php switch($record['return']): case "0": ?><button type="button" class="btn btn-xs btn-warning" disabled="disabled">未归还</button><?php break;?>
										<?php case "1": ?><button type="button" class="btn btn-xs btn-warning" disabled="disabled">未归还</button><?php break;?>
										<?php case "2": ?><button type="button" class="btn btn-xs btn-success" disabled="disabled">已归还</button><?php break; endswitch; break;?>
								<?php case "3": ?><button type="button" class="btn btn-xs btn-danger" disabled="disabled">审批驳回</button>
									<button type="button" class="btn btn-xs " style="visibility:hidden;">未归还</button><?php break; endswitch;?>
						</td>
						<td style="text-align: right;">
							...
							<?php if(($record['check_status'] == 2)): if(($record['return'] == 1)): ?><button type="button" class="btn btn-xs btn-danger btn-return">归还</button>
								<?php elseif(($record['return'] == 0)): ?>
									<button type="button" class="btn btn-xs btn-info btn-return">归还</button>
								<?php else: ?> 
									<button type="button" class="btn btn-xs" style="visibility:hidden;">归还</button><?php endif; ?>
							<?php else: ?> 
								<button type="button" class="btn btn-xs" style="visibility:hidden;">归还</button><?php endif; ?>
						</td>	
...
<script type="text/javascript">
	...
	var borrowList = <?php echo json_encode($borrowList);?>;
	$(function(){
		if(getCookie('page') != "home-borrow"){
		...
		$(".btn-edit").click(function(e){
			...
			editBorrow(borrowList[index]);
		});

		$(".btn-delete").click(function(e){
			var index = $(e.target).parent().parent().index();
			deleteBorrow(borrowList[index]);
		});

		$(".btn-view").click(function(e){
			var index = $(e.target).parent().parent().index();
			viewBorrow(borrowList[index]);
		});

		$(".btn-return").click(function(e){
			var index = $(e.target).parent().parent().index();
			returnBorrow(borrowList[index]);
		});
	});

	function onSearch(){
		$('#btn-search').button('loading');
		window.location.href = "<?php echo U('Main/borrow');?>"+"&"+$("#query-form").serialize();
	}
</script>